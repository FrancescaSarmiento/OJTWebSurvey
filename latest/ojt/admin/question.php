...
<?php
        if (isset($_POST["questionAdd"])){
            ...
            if($questionNo == '' || $questionDescription == ''){
                ...
                $msg=" Question Field is empty. Please Enter a value. ";
                    ...
                    if($questionNo == $number){
                        ...
                        if ($ntu_survey->query($sql) === TRUE) {
                            ...
                            foreach($choices as $ch => $value ){
                                ...
                                $msg="Please don't use single quotation mark.";
                        }
                                    ...
                                    <div class="col-lg-2">
                                        <?php
                                            if(isset($_POST['surveySubmit'])){
                                                ...
                                                $queryS="SELECT surveyId from survey where surveyTitle='$surveyTitle'";
                                                $resultS = mysqli_query($ntu_survey, $queryS);
                                                $rowS = $resultS->fetch_assoc();
                                                $surveyId = $rowS["surveyId"];
                                                
                                                $c = "SELECT COUNT(questionId) 'num' FROM question natural join survey where surveyId ='$urveyId'";
                                                $countQ = mysqli_query($ntu_survey, $c);
                                                $rowQ = $countQ->fetch_assoc();
                                                $count = $rowQ["num"];
                                                ...
                                                if($count > 0){
                                                    ...
                                                    $sql1 = "INSERT INTO surveylog (date, actionSurvey, user) VALUES (CONVERT_TZ('$date', '+00:00', '+8:00'),'Question/s for ". "$surveyTitle" ." has been Created','$userId')";  
                                                    ...
                                                    $date = date('Y-m-d H:i:s');
                                                    $sql1 = "INSERT INTO surveylog (date, actionSurvey, user) VALUES (CONVERT_TZ('$date', '+00:00', '+8:00'),'There are no Question/s created for ". "$surveyTitle" ." ','$userId')";  
